...
const ApartmentSignatureApp = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await window.storage.get('uploaded-pdf');
      if (result?.value) {
  ...
  const processFile = async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        await window.storage.set('uploaded-pdf', base64Data);
  ...
  const saveSig = async () => {
    ...
    try {
      ...
      await window.storage.set(`apartment:${currentApartment}`, JSON.stringify(sigData));
      ...
      await window.storage.set('notifications', JSON.stringify(updatedNotifs));
...
<body>
  ...
  const clearNotifications = async () => {
    await window.storage.set('notifications', JSON.stringify([]));
    setNotifications([]);
  };
